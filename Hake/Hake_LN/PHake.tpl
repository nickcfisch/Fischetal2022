//Recreation of Pacific Hake Assessment Model by N. Fisch July 2021
  ...
  //With Logistic-Normal AR1 for composition data 
...
PARAMETER_SECTION
  ...
  init_bounded_vector log_rec_devs(fyear-lage,recdev_end,-6.,6.,1) //recruitment deviations
  ...
  //LN Calcs
  init_bounded_number phi(-1.,1,5)                                 //Logistic-normal phi parameter for fishery comp
  init_bounded_number lsig_LN(-5.,5.,5)                            //Logistic-normal sigma parameter for fishery comp
  init_bounded_number phi_surv(-1.,1,5)                            //Logistic-normal phi parameter for survey comp
  init_bounded_number lsig_LN_surv(-5.,5.,5)                       //Logistic-normal sigma parameter for survey comp
...
//LN Variables
  3darray covmat(fyr_flt1,lyr_flt1,compfage,complage,compfage,complage)                  //Covariance Matrix for fishery comp LN
  matrix covmat_rho(compfage,complage,compfage,complage)
  matrix indicator_mat(compfage,complage,compfage,complage)
  matrix Kmat(compfage,complage-1,compfage,complage)
  3darray Vmat(fyr_flt1,lyr_flt1,compfage,complage-1,compfage,complage-1)
  3darray Vinv(fyr_flt1,lyr_flt1,compfage,complage-1,compfage,complage-1)
  vector wts(fyr_flt1,lyr_flt1)
  matrix ww(fyr_flt1,lyr_flt1,compfage,complage-1)
  vector sig_LN_yr(fyr_flt1,lyr_flt1)

  3darray covmat_surv(1,nyr_Acoustic,compfage+1,complage,compfage+1,complage)                  //Covariance Matrix for Survey comp LN
  matrix covmat_surv_rho(compfage+1,complage,compfage+1,complage)
  matrix indicator_mat_surv(compfage+1,complage,compfage+1,complage)
  matrix Kmat_surv(compfage+1,complage-1,compfage+1,complage)
  3darray Vmat_surv(1,nyr_Acoustic,compfage+1,complage-1,compfage+1,complage-1)
  3darray Vinv_surv(1,nyr_Acoustic,compfage+1,complage-1,compfage+1,complage-1)
  vector wts_surv(1,nyr_Acoustic)
  matrix ww_surv(1,nyr_Acoustic,compfage+1,complage-1)
  vector sig_LN_surv_yr(1,nyr_Acoustic)
...
PROCEDURE_SECTION
  ...
  if (mceval_phase())
    ...
    if (mycount==0){
     mcres << "log_R0 steepness log_M log_surv_sd  phi lsig_LN phi_surv lsig_LN_surv \
...
FUNCTION Logistic_Normal_calcs
  pi=3.141593;
  wts=sqrt(mean(SS_flt1)/SS_flt1);             //weights for each year of composition
...
//Code to generate Covariance matrix each year for multivariate normal variate x
   for(i=compfage;i<=complage;i++){
    for(j=compfage;j<=complage;j++){
  ...
  for(i=compfage;i<=complage-1;i++){
   Kmat(i,i)=1;                               //Making identity matrix
   Kmat(i,complage)=-1;                           //adding a column of -1's
  }
...
//Now code for NLL of logistic normal
  ...
  for(i=fyr_flt1;i<=lyr_flt1;i++){
   ...
   ww(i) = log(obs_fcomp(i)(compfage,complage-1)/obs_fcomp(i,complage)) - log(pred_fcomp_compressed(i)(compfage,complage-1)/pred_fcomp_compressed(i,complage));

   NLL1 += 0.5*(complage-compfage)*log(2*pi)+sum(log(obs_fcomp(i)))+0.5*log(det(Vmat(i)))+(complage-compfage)*log(wts(i))+0.5*((ww(i)*Vinv(i)*ww(i))/pow(wts(i),2))    ;
  }
...
//////////////////////////////////////////////////////
   wts_surv=sqrt(mean(SS_flt2)/SS_flt2);    //weights for each year of composition
   sig_LN_surv=mfexp(lsig_LN_surv);             //Exponentiating sigma
   sig_LN_surv_yr = wts_surv*sig_LN_surv;     //Getting sigmas for each year 

   for(i=compfage+1;i<=complage;i++){
    for(j=compfage+1;j<=complage;j++){
     indicator_mat_surv(i,j) = abs(int(i+1)-int(j+1));       //dummy matrix only needed for next calculation
     covmat_surv_rho(i,j) =  pow(phi_surv,indicator_mat_surv(i,j));       //covariance matrix of only rho
    }
   }

  for(i=compfage+1;i<=complage-1;i++){
   Kmat_surv(i,i)=1;                               //Making identity matrix
   Kmat_surv(i,complage)=-1;                           //adding a column of -1's
  }
  ...
  for(i=fyear;i<=lyear;i++){
   for(j=1;j<=nyr_Acoustic;j++){
    if(i==Acoustic_yrs(j)){
      covmat_surv(j) = covmat_surv_rho*pow(sig_LN_surv_yr(j),2);     //adding sig^2 into covariance matrix
      Vmat_surv(j) = Kmat_surv * (covmat_surv(j) * trans(Kmat_surv));        //Vmat calc
      Vinv_surv(j) = inv(Vmat_surv(j));                            //inverse of Vmat
      ww_surv(j) = log(obs_survcomp(j)(compfage+1,complage-1)/obs_survcomp(j,complage)) - log(pred_Survcomp_compressed(i)(compfage+1,complage-1)/pred_Survcomp_compressed(i,complage));

      NLL3 += 0.5*(complage+1-compfage)*log(2*pi)+sum(log(obs_survcomp(j)(compfage+1,complage)))+0.5*log(det(Vmat_surv(j)))+(complage+1-compfage)*log(wts_surv(j))+0.5*((ww_surv(j)*Vinv_surv(j)*ww_surv(j))/pow(wts_surv(j),2))    ;
     }
    }
   }
...
//Total NLL
  NLL=NLL1+NLL2+NLL3+NLL4+NLL5+NLL6+NLL7+NLLcatch;
  ...
  open_r_file("PHake.rdat");
   ...
   close_r_vector();
    ...
    open_r_vector("params");
      wrt_r_item("log_survq",log_survq);              //Survey Catchability 
      wrt_r_item("steepness",steepness);              //Steepness for recruitment
      wrt_r_item("log_R0",log_R0);                    
      wrt_r_item("log_surv_sd",log_surv_sd);          
      wrt_r_item("phi",phi);                          
      wrt_r_item("lsig_LN",lsig_LN);                  
      wrt_r_item("phi_surv",phi_surv);                
      wrt_r_item("lsig_LN_surv",lsig_LN_surv);         
     close_r_vector();
