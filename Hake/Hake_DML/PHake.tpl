//Recreation of Pacific Hake Assessment Model by N. Fisch July 2021
  ...
  //With Dirichlet-Multinomial Linear for composition data 
...
//Likelihood for Fishery Composition, Dirichlet Multinomial 
  ...
  //Linear Version from Thorson
   if(SS_flt1(i)>0){
    NLL1 += -1*(gammln(SS_flt1(i)+1)-sum(gammln(SS_flt1(i)*obs_fcomp(i)+ones))+gammln(fBeta(i))-gammln(SS_flt1(i)+fBeta(i))+sum(gammln(SS_flt1(i)*obs_fcomp(i)+fBeta(i)*pred_fcomp_compressed(i))-gammln(fBeta(i)*pred_fcomp_compressed(i)))); 
   }
  }
  ...
  for(i=fyear;i<=lyear;i++){
   for(j=1;j<=nyr_Acoustic;j++){
    if(i==Acoustic_yrs(j)){
     //Linear Version from Thorson
     if(SS_flt2(j)>0){
      NLL3 += -1*(gammln(SS_flt2(j)+1)-sum(gammln(SS_flt2(j)*obs_survcomp(j)(compfage+1,complage)+ones(compfage+1,complage)))+gammln(survBeta(j))-gammln(SS_flt2(j)+survBeta(j))+sum(gammln(SS_flt2(j)*obs_survcomp(j)(compfage+1,complage)+survBeta(j)*pred_Survcomp_compressed(i)(compfage+1,complage))-gammln(survBeta(j)*pred_Survcomp_compressed(i)(compfage+1,complage))));
     }
    }
   }
  }
