...
PROCEDURE_SECTION
  ...
  if (mceval_phase())
    {
//    ofstream mcres("SSBdivSSB0.txt",ios::app);
    if (mycount==0){
     }
       mycount++;
    }
...
//Likelihood for Fishery Composition, Dirichlet Multinomial 
  ...
  //Linear Version from Thorson
   if(SS_flt1(i)>0){
    NLL1 += -1*sum(SS_flt1(i)*elem_prod(obs_fcomp(i),log(.000001+pred_fcomp_compressed(i))));     //Likelihood for age composition of fishery catch
   }
  }
  ...
  for(i=fyear;i<=lyear;i++){
   for(j=1;j<=nyr_Acoustic;j++){
    if(i==Acoustic_yrs(j)){
     //Linear Version from Thorson
     if(SS_flt2(j)>0){
      NLL3 += -1*sum(SS_flt2(j)*elem_prod(obs_survcomp(j)(compfage+1,complage),log(.000001+pred_Survcomp_compressed(i)(compfage+1,complage))));     //Likelihood for age composition of fishery catch
     }
    }
   }
  }
